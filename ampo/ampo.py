...
class AMPO:
    ...
    def __init__(self, func, dim, bound, max_iters, pop=50,
        ...
        self.history = []
        self.gbest = {'individual': None,
                      'fitness': float('inf'), 'solution': None}
        self.migration_gbest = {'individual': None,
                                'fitness': float('inf'), 'solution': None}
    ...
    def run(self):
        ...
        for _ in range(self.main_pop_size):
            individuals.append(Individual(
                self.dim, self.bound, self.w, self.r))
        ...
        self.migration_solutions = np.array(
            [_individual.solution.solution for _individual in migrating_individuals])
        self.migration_solutions_two = np.array(
            [_individual.solution.solution for _individual in migrating_individuals])

###
        for iteration in range(self.iterations):
            # Remark: Use < -999999 for normal operation
            if self.gbest['fitness'] == -999999:
                print("----------STOP----------")
                break
            else:
                pbest = {'individual': None, 'fitness': float(
                    'inf'), 'solution': None}
                ...
                if pbest['individual'].fitness < self.gbest['fitness']:
                    existing_leader_individual = [
                        h for h in individuals if h.type == 'leader']
                ...
                source_individuals = [
                    individual for individual in individuals if individual.source_ind]
                random_individuals = [
                    individual for individual in individuals if not individual.source_ind]
                    ...
                    if len(random_individuals) >= 1:
                        ...
                        for healthy_individual in contacted_individuals:
                            ...
                            if type_prob <= trans_probs[trans_type]['global_search']:
                                healthy_individual.transform(
                                    source_individual, 'global_search')
                            else:
                                healthy_individual.transform(
                                    source_individual, 'local_search')
                ...
                self.history.append(
                    {'iteration': iteration, 'fitness': self.gbest['fitness']})

                self.fitness.append(self.gbest['fitness'])
        ...
        return self.gbest['solution'], self.gbest['fitness'], self.fitness
    ...
    def migrating_mutation(self, size_pop):
        ...
        mask = np.random.uniform(
            low=self.bound[0], high=self.bound[1], size=(size_pop, self.dim))
